...
const LocationConfig* HttpResponse::getMatchedLocation() const {
...
bool isPathUnderRoot(const std::string &root, const std::string &fullPath) {
    ...
    if (realpath(fullPath.c_str(), resolvedPath) == NULL) {
        std::string parent = fullPath;
        size_t slash = parent.rfind('/');
        if (slash == std::string::npos)
            return false;
        parent = parent.substr(0, slash);
        if (realpath(parent.c_str(), resolvedPath) == NULL)
            return false;
    }
 
    std::string normRoot(resolvedRoot);
    std::string normPath(resolvedPath);
    if (normRoot[normRoot.size()-1] != '/') normRoot += "/";
    if (normPath[normPath.size()-1] != '/') normPath += "/";
...
std::string HttpResponse::resolveFilePath(const HttpRequest& req, const ServerConfig& config) {
    ...
    for (size_t i = 0; i < config.locations.size(); i++) {
        ...
        if (req.path.substr(0, loc_path.size()) == loc_path) {
            // S'assurer que le match est sur une frontière propre :
            // le caractère suivant dans req.path doit être '/', fin de string,
            // ou loc_path se termine déjà par '/'
            size_t next = loc_path.size();
            bool boundary_ok = (next >= req.path.size())
                             || (req.path[next] == '/')
    ...
    if (root.empty())
        ...
        // Ajouter un slash entre root et suffix si nécessaire
...
std::string HttpResponse::handlePost(const HttpRequest& req, const std::string& path, const ServerConfig& config) {
    ...
    if (mp_pos != std::string::npos) {
        ...
        if (store_dir.empty() || !isDirectory(store_dir))
        ...
        while (true) {
            ...
            if (pos_body < body.size() && body[pos_body] == '\r') pos_body++;
            ...
            if (filename.empty()) { pos_body = part_end; continue; }
